...
with tab1:
    ...
    with col1:
        summary = st.text_area("Summary (optional - will auto-generate if empty)", 
                              height=80, max_chars=SUMMARY_MAX_LENGTH)
    with col2:
        char_count = len(summary) if summary else 0
        color = "red" if char_count > SUMMARY_MAX_LENGTH else "green"
        st.markdown(f"<p style='color: {color}; font-size: 12px;'>{char_count}/{SUMMARY_MAX_LENGTH} chars</p>", 
    ...
    if "loaded_note" in st.session_state:
        ...
        col1, col2 = st.columns([3, 1])
        with col1:
            new_summary = st.text_area("Edit Summary", 
        ...
        with col2:
            ...
            if st.button("🤖 Regenerate Summary", key="regen_summary", help="Auto-generate summary from current body text"):
                if new_body.strip():
